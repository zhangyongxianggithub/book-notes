...
### 传递规则
若关系$R$中的FD$A_{1},A_{2},\dots ,A_{n}\to B_{1},B_{2},\dots ,B_{m}$和$B_{1},B_{2},\dots ,B_{m}\to C_{1},C_{2},\dots ,C_{k}$都成立，那么$A_{1},A_{2},\dots ,A_{n}\to C_{1},C_{2},\dots ,C_{k}$也成立，可以通过计算闭包验证。当且仅当$A_{1},A_{2},\dots ,A_{n}$是关系的超键时，$\{A_{1},A_{2},\dots ,A_{n}\}^{+}$才是这个关系所有属性的集合，如果要验证$A_{1},A_{2},\dots ,A_{n}$是否是一个关系的超键，就看$\{A_{1},A_{2},\dots ,A_{n}\}^{+}$是否包含了全部的属性，然后移除任何一个属性后的集合$X$,$X^{+}$都不能包含全部的属性。
### 函数依赖的闭包集合
