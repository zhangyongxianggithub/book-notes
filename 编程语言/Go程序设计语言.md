...
## 恢复
退出程序是正确处理宕机的方式，也可以在退出前做清理工作。内置的`recover()`函数用于恢复，在延迟函数内部调用，并且包含defer的函数发生宕机，recover会终止当前的宕机状态并且返回宕机的值，函数不会从之前宕机的地方继续运行而是正常返回，recover在其他情况下运行没有任何效果且返回nil，类似线程中断。比如下面的代码:
```go
