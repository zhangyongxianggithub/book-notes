...
在使用`@ConfigurationProperties`时，Spring Boot会尝试将属性转化为正确的数据类型，如果你需要自定义的类型转换，你可以提供一个ConversionService bean(一个名叫conversionService的bean)，或者一个自定义属性编辑器(通过`CustomEditorConfigurer`bean)或者自定义`Converters`(使用注解`@ConfigurationPropertiesBinding`)